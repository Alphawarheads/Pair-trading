...
class PairTrading:
    ...
    def TradeSim(self, priceX, priceY, position, allocation):
        ...
        beta = 1  # Determine trading position: equal weight; delta hedge; capital ratio
        shareY = size * position  # shareY is the number of shares of the target
        shareX = [(-beta) * shareY.iloc[0] * priceY.iloc[0] / priceX.iloc[0]]
        cash = [2000 * allocation]  # Note the margin (20% margin, i.e., 5x leverage) and the market value of the pair trading (size*target price)

        for i in range(1, n):
            # Initialize the number of shares and cash balance for the current time step with the values from the previous time step
            shareX.append(shareX[i - 1])
            cash.append(cash[i - 1])

            # Adjust the number of shares and cash balance based on position changes
        ...
        # Convert to Pandas data structure and calculate total asset value
    ...
    def backtest_pair(self, pair, tradStart, tradEnd, allocation):
        ...
        spread_trade = self.SSD_Spread(price_priceX_trade, price_priceY_trade)
        mu = np.mean(spread_trade)
        sd = np.std(spread_trade)
        self.level = (
            float('-inf'), mu - 3.0 * sd, mu - 1.5 * sd, mu - 0.2 * sd, mu + 0.2 * sd, mu + 1.5 * sd, mu + 3.0 * sd,
            float('inf'))
        prcLevel = pd.cut(spread_trade, self.level, labels=False) - 3
        ...
        for i in range(1, ns):
            ...
            if signal[i] == 1:
                ...
                position[i] = 0
        position = pd.Series(position, index=spread_trade.index)
    ...
    def aggregate_backtests(self, tradStart, tradEnd):
        ...
        for pair, _ in self.pairs:
            ...
            print(account['Asset'])
            summary_stats = self.compute_summary_statistics(account)
            print(f"Summary statistics for {pair}:")
        ...
        # Print summary statistics for the combined account
        summary_stats = self.compute_summary_statistics(combined_account)
        print("Summary statistics for the combined account:")
        for key, value in summary_stats.items():
            print(f"{key}: {value}")
